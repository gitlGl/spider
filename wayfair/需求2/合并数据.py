...
def mer(sheet_names,colum_num):
   for sheet_name in sheet_names:
        server_file = f"{sheet_name}-row_data.csv"

        server_datas = get_data(server_file)
        local_datas = get_po_num(local_file,sheet_name,"B")
        ...
        for server_data in  server_datas:
            ...
            fill_green = PatternFill(fill_type='solid', fgColor='6495ED')  # 绿色填充
           
          
            if local_datas[index -1] is None:
                continue
           
            if server_data__ != local_datas[index -1]:
                # 创建填充对象并设置颜色
                #填充行为绿色
